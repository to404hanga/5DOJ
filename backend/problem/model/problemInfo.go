...
type ProblemInfo struct {
	...
	Title       string `gorm:"type:varchar(20);not null;index:idx_title;comment:'题目标题'"`                        // 题目标题
	...
	Enabled     bool   `gorm:"type:tinyint;not null;default:0;comment:'是否启用: 0=禁用, 1=启用'"`                      // 是否启用: 0=禁用, 1=启用
	...
	TotalScore  int    `gorm:"type:int;not null;default:null;comment:'总分数（仅 IOI 赛制有效）'"`                        // 总分数（仅 IOI 赛制有效）
